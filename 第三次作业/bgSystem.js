...
http.createServer(function(req,res){
    ...
    if(pathName == "/list"){
        ...
        var htmlContent = fs.readFileSync(path.join(__dirname,'chapter.html'));
        res.writeHead(200,{"Content-Type":"text/html;charset=utf-8"});
        res.end(htmlContent);
        ...
        var htmlContent = fs.readFileSync(path.join(__dirname,'list.html'));
        res.writeHead(200,{"Content-Type":"text/html;charset=utf-8"});
        res.end(htmlContent);
    }else if(pathName == '/login'){
        var htmlContent = fs.readFileSync(path.join(__dirname,'login.html'));
        ...
        var htmlContent = fs.readFileSync(path.join(__dirname,'addChapter.html'));
        res.writeHead(200,{"Content-Type":"text/html;charset=utf-8"});
        res.end(htmlContent);
        ...
        res.end(imgContent);
    }else if(pathName.indexOf("jpg") >=0 && req.method == 'GET'){
        var imgSrc = path.join(__dirname,pathName);
        var imgContent = fs.readFileSync(imgSrc);
        res.writeHead(200,{"Content-Type":"image/png"});
        res.end(imgContent);
...
function addChapter(req,res){
    ...
    req.on("end",function(){
        var dataObj = queryString.parse(userStr);
 ...
 *  访问特定的资源路径
 * 2、博客列表
 *  数据从服务端获取，jQuery ajax从服务端获取数据
 *  $.get("",function(data){
 * 
 * })
 * 3、阅读全文
 * http://localhost:8081/detail?chapterId=
 * 
 */